...
pyTRS is a library for parsing PLSS land descriptions. See more info at
<github.com/JamesPImes/pyTRS>
"""
...
def parse_tracts(df: pd.DataFrame, tract_col: str, suffix="_parsed") -> pd.DataFrame:
    ...
    atts = ["lotList", "QQList", "wFlagList", "eFlagList"]
    ...
    # DataFrame.
    parsed_tracts = [
        pyTRS.Tract(row[tract_col], initParseQQ=True).to_dict(atts)
...
def parse_plssdescs(
    ...
    # for our purposes.
    atts = [
        "source", "trs", "twp", "rge", "sec", "desc", "lotList",
        "QQList", "wFlagList", "eFlagList"
    ]
    ...
    list_type_atts = ["lotList", "QQList", "wFlagList", "eFlagList"]
    ...
    for ind, row in df.iterrows():
        ...
        dsc = pyTRS.PLSSDesc(
            row[plss_col], initParseQQ=True, config=config, source=ind)
...
def plssdesc_contains_trs(plssdesc_raw: str, trs, config=None) -> bool:
    ...
    if isinstance(trs, str):
        trs = [trs]
    trs = set([t.lower() for t in trs])
    dsc = pyTRS.PLSSDesc(plssdesc_raw, initParseQQ=True, config=config)
    found_trs = set(dsc.list_trs())
    return len(trs.intersection(found_trs)) > 0
